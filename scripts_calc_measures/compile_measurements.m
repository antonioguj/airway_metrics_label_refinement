function compileMeasurements ()
    ...
    if ~isdeployed
        addpath('/home/agarcia/Codes/airway_measures/src_matlab/functions/');
        addpath('/home/agarcia/Codes/airway_measures/src_matlab/scripts/');
    end

    % ----- load configurations -----------
    baseDataDir                	= '/scratch/agarcia/Tests/LabelRefinement_THIRONA/'

    cfg.files.folderCT         	= [baseDataDir '/CTs/'];
    cfg.files.folderOpfronted 	= [baseDataDir '/Segmentations/'];
    cfg.files.saveFileResults	= [baseDataDir '/Measurements/%s_ResultsPerBranch.csv'];
...
%% ------------------------------- AUXILIARY FUNCTIONS -------------------------------------------
function saveResultsInCSV( data, fileList, id_images_processed, templ_str_file_out )
	...
	str_file_out = strrep(templ_str_file_out, '%s', image_id);
